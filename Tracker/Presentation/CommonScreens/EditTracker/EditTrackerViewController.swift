...
final class EditTrackerViewContoller: UIViewController, NewCategoryDelegateProtocol, ScheduleDelegateProtocol {
    ...
    private var completedDaysLabelConstraint: [NSLayoutConstraint] {
        [
            ...
            completedDaysLabel.topAnchor.constraint(equalTo: view.layoutMarginsGuide.topAnchor, constant: 24),
    ...
    private var textFieldVStackConstraint: [NSLayoutConstraint] {
        [
            ...
            textFieldVStack.topAnchor.constraint(equalTo: isEvent ?? true ? view.layoutMarginsGuide.topAnchor : completedDaysLabel.bottomAnchor, constant: isEvent ?? true ? 40 : 24),
    ...
    private var newTrackersTableConstraint: [NSLayoutConstraint] {
        [
            ...
            newTrackersTable.heightAnchor.constraint(equalToConstant: isEvent ?? true ? 75 : 150)
        ]
    }
    ...
    private var allUiElementsArray: [UIView] {
        [completedDaysLabel,cancelButton,makeTrackerButton,textFieldVStack,newTrackersTable,newTrackerCollection]
    }
    private var allConstraintsArray: [NSLayoutConstraint] {
        completedDaysLabelConstraint + cancelButtonConstraint + makeTrackerButtonConstraint + textFieldVStackConstraint + newTrackersTableConstraint + newTrackerCollectionViewConstraint
    }
    ...
    private var originalTrackerCategory: TrackerCategory?
    private let uiColorMarshalling = UIColorMarshalling()
    private var isEvent: Bool?
    // MARK: - Initializers
    init(selected: Tracker, selectedCategory: TrackerCategory?, completedDays: String) {
        super.init(nibName: nil, bundle: nil)
        tracker = selected
        originalTrackerCategory = selectedCategory
        selectedWeekDays = selected.schedule
        selectedColorIndex = colorArr.firstIndex(where: { uiColorMarshalling.hexString(from: $0) == uiColorMarshalling.hexString(from: selected.color )})
        selectedEmojiIndex = emojiArr.firstIndex(of: selected.emoji)
        selectedNewTrackerTitle = selected.name
        self.completedDaysLabel.text = completedDays
        self.selectedCategory = selectedCategory
        isEvent = selected.isEvent
    }
    ...
    override func viewDidLoad() {
        ...
        textField.text = selectedNewTrackerTitle
        self.setUI(to: allUiElementsArray, set: allConstraintsArray)
        completedDaysLabel.isHidden = isEvent ?? true
        newTrackerCollection.reloadData()
        setupSelectedCells()
    }
    ...
    func makeTrackerButtonTapped() {
        guard let selectedNewTrackerTitle,
              let selectedCategory,
              let selectedColorIndex,
              let selectedEmojiIndex,
              let originalTracker = tracker else { return }

        let hasChanged = selectedNewTrackerTitle != originalTracker.name ||
                         colorArr[selectedColorIndex] != originalTracker.color ||
                         emojiArr[selectedEmojiIndex] != originalTracker.emoji ||
                         selectedWeekDays != originalTracker.schedule ||
                         selectedCategory.name != originalTrackerCategory?.name
        ...
        try? trackerStorage.deleteTracker(originalTracker)
        let newTracker = Tracker(
            ...
            isEvent: originalTracker.isEvent,
            isPinned: originalTracker.isPinned
        )
        trackerStorage.createNewTracker(tracker: newTracker, category: selectedCategory.name)
    ...
    private func setupSelectedCells() {
        if let selectedEmojiIndex {
            let emojiIndexPath = IndexPath(row: selectedEmojiIndex, section: 0)
            newTrackerCollection.selectItem(at: emojiIndexPath, animated: false, scrollPosition: .init())
        }
        
        if let selectedColorIndex {
            let colorIndexPath = IndexPath(row: selectedColorIndex, section: 1)
            newTrackerCollection.selectItem(at: colorIndexPath, animated: false, scrollPosition: .init())
        }
    }
}

extension EditTrackerViewContoller: UITextFieldDelegate {
...
extension EditTrackerViewContoller: UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return isEvent ?? true ? trackersTableSections.count - 1 : trackersTableSections.count
    }
...
extension EditTrackerViewContoller: UITableViewDelegate {
...
extension EditTrackerViewContoller: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        case 0:
            ...
            if let selectedEmojiIndex, selectedEmojiIndex == indexPath.row {
                cell.selectEmojiCell()
            }
        case 1:
            cell.configureColorCell(color: colorArr[indexPath.row])
            if let selectedColorIndex, selectedColorIndex == indexPath.row {
...
extension EditTrackerViewContoller: UICollectionViewDelegateFlowLayout {
...
extension EditTrackerViewContoller: UICollectionViewDelegate {
